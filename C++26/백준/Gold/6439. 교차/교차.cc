...
int main(void)
{
	...
	while (t--)
	{
		...
		if (min(r[0].first, r[1].first) <= min(l[0].first, l[1].first) && max(l[0].first, l[1].first) <= max(r[0].first, r[1].first) &&
			min(r[0].second, r[1].second) <= min(l[0].second, l[1].second) && max(l[0].second, l[1].second) <= max(r[0].second, r[1].second))
		{
			cout << "T\n";
			continue;
		}
		...
		ch = cross(l[0], l[1], r[0], r[2]) * cross(l[0], l[1], r[0], r[3]) * cross(l[0], l[1], r[1], r[2]) * cross(l[0], l[1], r[1], r[3]);
		if (ch)
			cout << "F\n";
		else
			cout << "T\n";
